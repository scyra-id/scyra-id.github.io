...
function setupManualModal() {
    ...
    document.getElementById('materiForm').onsubmit = async (e) => {
        ...
        try {
            if (id) {
                await window.db.from('materi').update({ judul, kategori_id, status, konten_html, nomor_bab }).eq('id', id);
            } else {
                const { data: { user } } = await window.db.auth.getUser();
                await window.db.from('materi').insert({ judul, slug, kategori_id, status, konten_html, nomor_bab, created_by: user.id });
            }
