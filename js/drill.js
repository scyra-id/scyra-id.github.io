     ...
     // 🚨 SATPAM RBAC: CEK HAK AKSES BAB (ANTI-BYPASS URL)
        try {
         ...
         // B. Pastikan Role User sudah terdeteksi (Fallback fetch kalau session.js telat load)
            ...
            const isGold = (currentRole === 'gold' || currentRole === 'admin');
            const isUserFree = (currentRole === 'user' || currentRole === 'guest');
         
         // C. Cek apakah ada bab premium (nomor_bab > 1) yang diminta
         // Kalau user minta campuran (Bab 1 + Bab 2), dan dia Free, cek dulu apakah sudah unlock dengan credit
            const premiumBab = materiList.find(m => (m.nomor_bab || 1) > 1);

            if (isUserFree && premiumBab) {
                // Cek apakah user sudah unlock subtest ini dengan credit
                let isSubtestUnlocked = false;
                const needsCredit = window.CreditSystem ? await window.CreditSystem.needsCreditSystem() : false;
                
                if (needsCredit && window.CreditSystem) {
                    // Ambil kategori_id dari salah satu materi untuk cek unlock status
                    const { data: materiDetail } = await window.db
                        .from('materi')
                        .select('kategori_id')
                        .eq('id', ids[0])
                        .single();
                    ...
                    if (materiDetail) {
                        isSubtestUnlocked = await window.CreditSystem.isContentUnlocked('subtest', materiDetail.kategori_id);
                    }
                }
                
                // Jika belum unlock dengan credit, blokir akses ke bab premium
                if (!isSubtestUnlocked) {
                    await showScyraAlert(
                        `Drill soal untuk <strong>${premiumBab.judul}</strong> adalah konten Premium.<br><br>🎯 <strong>Bab 1 gratis untuk semua user!</strong><br><br>Untuk akses bab lainnya, unlock subtest dengan 20 ${CREDIT_ICON} credit atau upgrade ke Silver/Gold!`,
                        '🔒 Akses Ditolak', 
                        '🔒'
                    );
                    window.location.href = 'materi.html';
